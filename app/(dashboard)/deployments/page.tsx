  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <button onClick={() => setShowNewDeployment(true)} className="flex items-center gap-2 bg-[#00E5FF] hover:bg-[#00E5FF]/90 text-slate-900 dark:text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-white/10 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-t border-slate-200 dark:border-white/5 flex justify-end gap-3">
              ...
              <button onClick={handleCreateDeployment} className="px-4 py-2 text-sm font-medium bg-[#00E5FF] text-slate-900 rounded-lg hover:bg-[#00E5FF]/90 transition-colors">
                Deploy Now
              </button>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {deployments.map(dep => (
          <div key={dep.id} className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-white/5 rounded-xl p-6 hover:-translate-y-1 hover:shadow-lg hover:border-slate-300 dark:hover:border-white/10 transition-all duration-300 cursor-pointer group flex flex-col justify-between">
            <div>
              <div className="w-10 h-10 rounded-lg bg-slate-50 dark:bg-[#020617] border border-slate-200 dark:border-white/5 flex items-center justify-center mb-4">
                <dep.icon size={18} className="text-sky-600 dark:text-[#00E5FF]" />
              </div>
              <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1.5 flex items-center gap-2">
                ...
                <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded flex items-center gap-1.5 ${
                    ...
                    dep.status === 'Deploying' ? 'bg-sky-100 dark:bg-[#00E5FF]/10 text-sky-600 dark:text-[#00E5FF]' :
                    'bg-red-100 dark:bg-[#EF4444]/10 text-red-500 dark:text-[#EF4444]'
                }`}>
                  {(dep.status === 'Healthy' || dep.status === 'Deploying') && <span className={`w-1.5 h-1.5 rounded-full ${dep.status === 'Healthy' ? 'bg-[#10B981]' : 'bg-[#00E5FF]'} animate-pulse`}></span>}
                  {dep.status}
                </span>
              </h3>
              <p className="text-[13px] text-slate-500 dark:text-[#888] mb-2">{dep.type}</p>
              <a href="#" className="text-[13px] text-sky-600 dark:text-[#00E5FF] hover:underline mb-6 inline-block truncate max-w-full">
                {dep.url}
              </a>
            </div>
            <div className="text-[12px] text-slate-400 dark:text-[#666] font-medium flex items-center justify-between border-t border-slate-200 dark:border-white/5 pt-4">
              ...
              <span className="text-sky-600 dark:text-[#00E5FF] group-hover:text-sky-600 dark:text-[#00E5FF]/80 transition-colors text-sm font-semibold">Manage</span>
            </div>
          </div>
        ))}
      </div>
